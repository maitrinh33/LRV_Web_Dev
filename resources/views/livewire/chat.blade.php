                ...
                <div class="w-full px-5 py-8">
                    <div class="grid gap-4">
                        @foreach($users as $user)
                            <a href="{{ route('chats.view', $user->id) }}" class="flex items-center p-4 bg-white rounded-lg shadow hover:bg-gray-50 transition-colors">
                                <img src="https://pagedone.io/asset/uploads/1710412177.png" alt="{{ $user->name }}" class="w-12 h-12 rounded-full">
                                <div class="ml-4 flex-1">
                                    <h3 class="text-lg font-semibold text-gray-900">{{ $user->name }}</h3>
                                    @if($user->unread_messages_count > 0)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                                            {{ $user->unread_messages_count }} new messages
                                        </span>
                                    @endif
                                </div>
                            </a>
